...
### SEO et URLs publiques — garde-fous durables

- Une modification technique ne doit pas supprimer ou dégrader involontairement les URLs publiques, `title`, meta description, canonical, `og:url`, H1/Hn (un seul `h1` pertinent), données structurées, contenu indexable ou maillage interne. Les améliorations SEO volontaires sont permises dans le périmètre explicitement autorisé ; ne pas désindexer involontairement une page publique indexable.
- Ne modifier `.htaccess`, `sitemap.xml`, le pré-rendu, les slugs, redirections, canonicals ou domaine que dans un périmètre SEO explicitement autorisé par Thierry, y compris au sein d'un chantier d'ergonomie.
- Domaine canonique : `https://formaprompt.com`, HTTPS sans `www`. Préserver `/privacy` → `/politique-confidentialite`, `/contact.html` → `/contact`, `/formation-ia-formateur` → `/formation-organismes` en `301`, et le Studio canonique `https://formaprompt.com/studio/`. Une URL publique inconnue doit répondre réellement `404`, sans accueil de substitution.
- Pour les pages canoniques indexables, viser un `200` direct. Une ancienne URL peut rediriger en `301` vers sa destination canonique pertinente ; éviter les chaînes multiples, interdire les boucles et les redirections vers une page sans rapport pour masquer une `404`. Ne pas généraliser cette règle à toutes les ressources techniques.
- Pour les pages indexables présentes dans le sitemap, faire correspondre URL du sitemap, URL finale et canonical. Ne pas ajouter les anciennes URLs redirigées comme canonicals concurrentes, ni forcer les pages techniques, privées ou non indexables dans le sitemap.
- Historique de régression : la boucle `/contact` et le conflit `/blog`–`/blog/` ont été corrigés dans `origin/main` par `d67c0459a1241567348999f10a0c40017711fd59` (routes contact/blog) et `123ae23d0f79b31b268467a729f706cc6064b8cc` (DirectorySlash). Conserver `/contact`, `/contact/`, `/contact.html`, `/blog` et `/blog/` comme cas de test, avec destinations canoniques `/contact` et `/blog`. Ces références ne décrivent pas l'état courant de production.
- Vérifier les interactions entre dossiers physiques, routes applicatives et pré-rendu : Apache `DirectorySlash` peut ajouter automatiquement un slash lorsqu'une route, par exemple `/blog`, correspond aussi à un répertoire. Ne pas appliquer `DirectorySlash Off` globalement par réflexe ; justifier sa portée par l'architecture réelle et contrôler les routes et dossiers concernés.
- Dans `.htaccess`, distinguer redirections externes permanentes et normalisation canonique, exceptions/routes spécifiques, réécritures internes vers les pages générées, traitement des fichiers/dossiers physiques, fallback applicatif et vraie `404`. Vérifier leur ordre selon l'architecture : aucune règle générale, test de dossier ou fallback ne doit court-circuiter une exception ; les réécritures internes ne doivent pas relancer une redirection externe en boucle. Limiter le fallback aux routes prévues.
- Avant commit, lorsque possible : exécuter build, pré-rendu et tests pertinents ; inspecter les routes et HTML générés, canonicals et sitemap ; rechercher les boucles localement. Un serveur Vite ne valide pas les règles Apache : tester celles-ci sur un environnement Apache compatible disponible, sinon signaler cette limite. Ne pas exiger de la production qu'elle valide un changement local non déployé.
- Après un déploiement distinctement autorisé : contrôler sur les URLs réellement publiées le code HTTP final, chaque étape de redirection, l'absence de boucle, la canonical et sa cohérence avec le sitemap pour les pages indexables concernées. Tester les variantes avec/sans slash pertinentes, les anciennes URLs et une URL inconnue ; consigner attendu/observé. Ne pas assimiler un build réussi à une validation HTTP de production.
