...
### SEO public — invariants de production

- La source des règles Apache est `webapp/public/.htaccess`. `webapp/dist/.htaccess` est généré par le build et ne doit jamais être modifié seul.
- Conserver le domaine canonique unique `https://formaprompt.com`, sans `www`, avec HTTPS forcé.
- Conserver les redirections permanentes `/privacy` → `/politique-confidentialite`, `/contact.html` → `/contact` et `/formation-ia-formateur` → `/formation-organismes`.
- Conserver les canonicals `https://formaprompt.com/politique-confidentialite` et `https://formaprompt.com/studio/`.
- Les URL publiques inconnues doivent répondre avec une vraie erreur HTTP `404`, sans copie silencieuse de l'accueil.
- Préserver dans `.htaccess` la protection de l'accès direct aux médias payants.
- Avant de modifier `.htaccess`, les canonicals, le sitemap, le pré-rendu public, les routes publiques ou leurs redirections : expliquer l'impact SEO et obtenir l'accord explicite de Thierry si un invariant doit changer.
- Après une modification autorisée : vérifier que `public/.htaccess` et `dist/.htaccess` sont identiques après build, puis tester les codes HTTP, les destinations de redirection et les balises canoniques concernés.
- Une alerte Search Console concernant une ancienne URL ne justifie pas de la rétablir en `200` : conserver sa redirection vers l'URL canonique prévue.
