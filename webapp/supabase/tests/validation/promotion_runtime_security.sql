...
BEGIN
  ...
  FOREACH v_function IN ARRAY ARRAY[
    ...
    'public.prepare_diagnostic_promotion_checkout(uuid,uuid,text,text)'::regprocedure,
    'public.reset_diagnostic_promotion_checkout(uuid,uuid)'::regprocedure,
    'public.process_diagnostic_ia_stripe_event(jsonb)'::regprocedure,
  ...
  FOREACH v_function IN ARRAY ARRAY[
    ...
    'private.prepare_diagnostic_promotion_checkout(uuid,uuid,text,text)'::regprocedure,
    'private.reset_diagnostic_promotion_checkout(uuid,uuid)'::regprocedure
  ]
  LOOP
    IF has_function_privilege('anon', v_function, 'EXECUTE')
      OR has_function_privilege('authenticated', v_function, 'EXECUTE')
...
BEGIN
  ...
  FROM public.admin_create_promotion(
    ...
    '[{"target_type":"diagnostic","target_key":"diagnostic-ia-express"}]'::jsonb
  );
  PERFORM public.admin_update_promotion(
