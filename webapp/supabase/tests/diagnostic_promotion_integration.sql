...
-- Cas additionnels 1G-B : fixtures locales, jamais un code de production.
RESET ROLE;
INSERT INTO public.promo_codes(id,code,discount_type,discount_value,active,starts_at,max_uses_per_user)
VALUES
 ('98000000-0000-4000-8000-000000000109','LOCAL_FUTURE','percent',10,true,now()+interval '1 day',NULL),
 ('98000000-0000-4000-8000-000000000110','LOCAL95','percent',95,true,NULL,1),
 ('98000000-0000-4000-8000-000000000111','LOCAL_ZERO','fixed_amount',14900,true,NULL,NULL);
INSERT INTO public.promo_code_targets(promo_code_id,target_type,target_key)
SELECT id,'diagnostic','diagnostic-ia-express' FROM public.promo_codes
WHERE id IN ('98000000-0000-4000-8000-000000000109','98000000-0000-4000-8000-000000000110','98000000-0000-4000-8000-000000000111');
INSERT INTO public.diagnostic_ia_orders(id,user_id,customer_email,sales_context,cgv_document_version_id,cgv_acceptance_statement_version_id)
VALUES ('98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','personal',
 (SELECT id FROM public.legal_document_versions WHERE version='CGV-B2C-2026-08-26'),
 (SELECT id FROM public.legal_document_versions WHERE version='DIAGNOSTIC-CGV-ACCEPTANCE-2026-08-26'));
SET LOCAL ROLE service_role;
SELECT throws_ok($$SELECT * FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','LOCAL_ZERO')$$,
 'P0001','Ce code n''est pas valide ou n''est plus disponible.','Un Diagnostic gratuit est refuse');
SELECT is((SELECT count(*)::integer FROM public.promo_redemptions WHERE order_context_id='98000000-0000-4000-8000-000000000207'),0,
 'Le refus zero annule la reservation dans la meme transaction');
SELECT ok((SELECT checkout_configuration_locked_at IS NULL FROM public.diagnostic_ia_orders WHERE id='98000000-0000-4000-8000-000000000207'),
 'Le refus zero ne fige pas la commande');
SELECT throws_ok($$SELECT * FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','LOCAL_FUTURE')$$,
 'P0001','Ce code n''est pas valide ou n''est plus disponible.','Un code pas encore actif est refuse');
SELECT throws_ok($$SELECT * FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','bad code!')$$,
 'P0001','Ce code n''est pas valide ou n''est plus disponible.','Un code mal forme est refuse');
SELECT is((SELECT final_amount_cents FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test',' local95 ')),745,
 'Le moteur reel calcule 149 euros moins 95 pour cent soit 7.45 euros');
SELECT is((SELECT discount_amount_cents FROM public.diagnostic_ia_orders WHERE id='98000000-0000-4000-8000-000000000207'),14155,
 'La remise serveur vaut exactement 14155 centimes');
SELECT is((SELECT final_amount_cents FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','LOCAL95')),745,
 'Le retry identique conserve le prix fige');
SELECT is((SELECT count(*)::integer FROM public.promo_redemptions WHERE order_context_id='98000000-0000-4000-8000-000000000207'),1,
 'Le retry ne cree aucune seconde reservation');
SELECT throws_ok($$SELECT * FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test',NULL)$$,
 'P0001','Ce code n''est pas valide ou n''est plus disponible.','Retirer un code fige est refuse');
SELECT throws_ok($$SELECT * FROM public.prepare_diagnostic_promotion_checkout(
 '98000000-0000-4000-8000-000000000207','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','DIAG10')$$,
 'P0001','Ce code n''est pas valide ou n''est plus disponible.','Changer un code fige est refuse');
SELECT throws_ok($$SELECT * FROM public.validate_promo_code_for_checkout(
 'LOCAL95','98000000-0000-4000-8000-000000000004','promo-diag-d@example.test','diagnostic','diagnostic-ia-express',14900)$$,
 'P0001','Ce code n''est pas valide ou n''est plus disponible.','Le quota utilisateur compte la reservation active');
RESET ROLE;
...
-- Controles contractuels transactionnels supplementaires : une exception fait
-- echouer la suite, sans modifier l inventaire des 55 assertions promotionnelles.
SET LOCAL ROLE service_role;
DO $$
DECLARE
  v_order public.diagnostic_ia_orders%ROWTYPE;
  v_proof public.diagnostic_ia_consents%ROWTYPE;
  v_version text;
BEGIN
  SELECT * INTO STRICT v_order FROM public.diagnostic_ia_orders
    WHERE id = '98000000-0000-4000-8000-000000000207';
  SELECT version INTO STRICT v_version FROM public.legal_document_versions
    WHERE id = v_order.cgv_acceptance_statement_version_id;
  IF v_version <> 'DIAGNOSTIC-CGV-ACCEPTANCE-PROMO-2026-09-03' THEN
    RAISE EXCEPTION 'Modele promotionnel non fige';
  END IF;
  INSERT INTO public.diagnostic_ia_consents
    (order_id,user_id,consent_type,legal_document_version_id,granted,source,acceptance_text)
  VALUES (v_order.id,v_order.user_id,'cgv_acceptance',v_order.cgv_acceptance_statement_version_id,
    true,'web_checkout','Texte client non autoritatif') RETURNING * INTO v_proof;
  IF v_proof.acceptance_text IS DISTINCT FROM
    'J''accepte les Conditions générales de vente applicables au Diagnostic IA Express. Prix catalogue : 149,00 €. Remise promotionnelle : 141,55 €. Montant total de ma commande : 7,45 €. Je reconnais que ma commande m''oblige au paiement de 7,45 €.'
    OR v_proof.created_at IS NULL THEN RAISE EXCEPTION 'Preuve remisee incorrecte'; END IF;
  BEGIN
    UPDATE public.diagnostic_ia_consents SET acceptance_text='Paiement de 149 €' WHERE id=v_proof.id;
    RAISE EXCEPTION 'Modification de preuve acceptee a tort';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  IF private.diagnostic_promo_acceptance_text(14900,1490,13410,'eur') NOT LIKE '%paiement de 134,10 €.'
    OR private.diagnostic_promo_acceptance_text(14900,2000,12900,'eur') NOT LIKE '%paiement de 129,00 €.' THEN
    RAISE EXCEPTION 'Preuve non dynamique';
  END IF;
  SELECT * INTO STRICT v_order FROM public.diagnostic_ia_orders
    WHERE id='98000000-0000-4000-8000-000000000204';
  INSERT INTO public.diagnostic_ia_consents
    (order_id,user_id,consent_type,legal_document_version_id,granted,source)
  VALUES (v_order.id,v_order.user_id,'cgv_acceptance',v_order.cgv_acceptance_statement_version_id,true,'web_checkout')
    RETURNING * INTO v_proof;
  SELECT version INTO STRICT v_version FROM public.legal_document_versions WHERE id=v_proof.legal_document_version_id;
  IF v_version <> 'DIAGNOSTIC-CGV-ACCEPTANCE-2026-08-26' OR v_proof.acceptance_text IS NOT NULL
    OR v_order.amount_total <> 14900 OR v_order.promo_redemption_id IS NOT NULL THEN
    RAISE EXCEPTION 'Preuve historique sans remise modifiee';
  END IF;
END;
$$;
RESET ROLE;
