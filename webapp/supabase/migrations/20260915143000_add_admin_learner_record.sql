-- Fiche apprenant administrative consolidee, strictement en lecture seule.
-- Le controle historique du personnel inclut les employes : ce contrat
-- reutilise donc le controle admin strict avant toute lecture privilegiee.
BEGIN;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Apprenant introuvable.' USING ERRCODE = 'P0002';
  END IF;
...
REVOKE ALL ON FUNCTION public.admin_get_learner_record(uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_get_learner_record(uuid) TO authenticated;

COMMENT ON FUNCTION public.admin_get_learner_record(uuid) IS
  'Lecture minimisee d une fiche apprenant par UUID exact, reservee au role admin strict. Les notes et retours confidentiels repondent au besoin explicite de suivi de la fiche.';
