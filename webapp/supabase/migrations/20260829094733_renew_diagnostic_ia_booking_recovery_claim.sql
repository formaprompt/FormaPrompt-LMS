...
DECLARE
  ...
  v_recoverable_slot_count integer;
BEGIN
  ...
  FROM (
    ...
    WHERE bookings.status = 'booking_pending'
      ...
      AND bookings.order_id <> p_order_id
  ...
  IF FOUND THEN
    ...
    IF v_booking.status = 'booking_pending' THEN
      IF coalesce(v_existing_slot_ids, ARRAY[]::uuid[]) <@ p_slot_ids
        AND p_slot_ids <@ coalesce(v_existing_slot_ids, ARRAY[]::uuid[])
      THEN
        SELECT count(*)::integer INTO v_recoverable_slot_count
        FROM public.diagnostic_ia_booking_slots AS links
        JOIN public.training_availability_slots AS availability
          ON availability.id = links.availability_slot_id
        WHERE links.booking_id = v_booking.id
          AND links.released_at IS NULL
          AND availability.is_active
          AND availability.is_reserved;

        IF v_recoverable_slot_count = 3 THEN
          UPDATE public.diagnostic_ia_bookings AS bookings
