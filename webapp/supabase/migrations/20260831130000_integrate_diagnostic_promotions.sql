...
-- Modele transactionnel distinct : ni nouvelle CGV, ni remplacement de la preuve sans remise.
ALTER TABLE public.legal_document_versions
  ...
  ADD CONSTRAINT legal_document_versions_type_check CHECK (document_type IN (
    'cgv_b2c', 'cgv_b2b', 'privacy_policy', 'internal_rules', 'precontractual_information',
    'early_service_start_statement', 'digital_content_start_statement',
    'digital_content_withdrawal_acknowledgement', 'diagnostic_cgv_acceptance_statement',
    'diagnostic_early_service_start_statement', 'diagnostic_full_performance_withdrawal_acknowledgement',
    'withdrawal_form', 'diagnostic_promo_cgv_acceptance_statement'
  ));

WITH template(content) AS (VALUES (
  'J''accepte les Conditions générales de vente applicables au Diagnostic IA Express. Prix catalogue : {catalogue}. Remise promotionnelle : {remise}. Montant total de ma commande : {total}. Je reconnais que ma commande m''oblige au paiement de {total}.'
))
INSERT INTO public.legal_document_versions
  (document_type, version, status, content_text, content_sha256, effective_at)
SELECT 'diagnostic_promo_cgv_acceptance_statement', 'DIAGNOSTIC-CGV-ACCEPTANCE-PROMO-2026-09-03',
  'published', content, encode(extensions.digest(convert_to(content, 'UTF8'), 'sha256'), 'hex'), now()
FROM template;

-- Tous les autres elements de preuve existent deja : document FK, commande
-- (montants/devise/redemption), user_id, granted et created_at PostgreSQL.
ALTER TABLE public.diagnostic_ia_consents ADD COLUMN acceptance_text text;
COMMENT ON COLUMN public.diagnostic_ia_consents.acceptance_text IS
  'Texte promo rendu et fige par PostgreSQL depuis la commande. NULL pour les preuves historiques.';

CREATE FUNCTION private.diagnostic_promo_acceptance_text(p_original integer, p_discount integer, p_final integer, p_currency text)
RETURNS text LANGUAGE plpgsql STABLE SET search_path = '' AS $$
DECLARE
  v_template text;
BEGIN
  IF p_original IS DISTINCT FROM 14900 OR p_currency IS DISTINCT FROM 'eur'
    OR p_discount IS NULL OR p_discount <= 0 OR p_final IS NULL OR p_final <= 0
    OR p_discount + p_final <> p_original THEN
    RAISE EXCEPTION 'Preuve transactionnelle Diagnostic incoherente.' USING ERRCODE = '23514';
  END IF;
  SELECT content_text INTO STRICT v_template FROM public.legal_document_versions
  WHERE document_type = 'diagnostic_promo_cgv_acceptance_statement'
    AND version = 'DIAGNOSTIC-CGV-ACCEPTANCE-PROMO-2026-09-03' AND status = 'published';
  RETURN replace(replace(replace(v_template,
    '{catalogue}', (p_original / 100)::text || ',' || lpad((p_original % 100)::text, 2, '0') || ' €'),
    '{remise}', (p_discount / 100)::text || ',' || lpad((p_discount % 100)::text, 2, '0') || ' €'),
    '{total}', (p_final / 100)::text || ',' || lpad((p_final % 100)::text, 2, '0') || ' €');
END;
$$;
REVOKE ALL ON FUNCTION private.diagnostic_promo_acceptance_text(integer,integer,integer,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.diagnostic_promo_acceptance_text(integer,integer,integer,text) TO service_role;

CREATE OR REPLACE FUNCTION private.validate_diagnostic_ia_consent()
...
BEGIN
  IF TG_OP = 'UPDATE' AND OLD.acceptance_text IS NOT NULL THEN
    RAISE EXCEPTION 'Une preuve transactionnelle est figee.' USING ERRCODE = '42501';
  END IF;
  SELECT * INTO v_order FROM public.diagnostic_ia_orders WHERE id = NEW.order_id;
  ...
  IF v_order.user_id IS DISTINCT FROM NEW.user_id OR v_document_status IS DISTINCT FROM 'published' THEN
    RAISE EXCEPTION 'Preuve de consentement Diagnostic IA incoherente.' USING ERRCODE = '23514';
  END IF;
  IF (NEW.consent_type = 'cgv_acceptance' AND v_document_type NOT IN
      ('diagnostic_cgv_acceptance_statement', 'diagnostic_promo_cgv_acceptance_statement'))
    ...
    OR (NEW.consent_type = 'full_performance_withdrawal_acknowledgement'
      AND v_document_type <> 'diagnostic_full_performance_withdrawal_acknowledgement') THEN
    RAISE EXCEPTION 'Version juridique incorrecte pour le consentement Diagnostic IA.' USING ERRCODE = '23514';
  END IF;
  IF NEW.consent_type <> 'cgv_acceptance' AND v_order.sales_context <> 'personal' THEN
    RAISE EXCEPTION 'Le consentement de retractation anticipee est reserve au parcours B2C.' USING ERRCODE = '23514';
  END IF;
  IF NEW.consent_type = 'cgv_acceptance' AND v_order.promo_redemption_id IS NOT NULL THEN
    IF v_document_type <> 'diagnostic_promo_cgv_acceptance_statement'
      OR NEW.legal_document_version_id IS DISTINCT FROM v_order.cgv_acceptance_statement_version_id
      OR v_order.checkout_configuration_locked_at IS NULL THEN
      RAISE EXCEPTION 'Preuve transactionnelle Diagnostic incoherente.' USING ERRCODE = '23514';
    END IF;
    NEW.acceptance_text := private.diagnostic_promo_acceptance_text(
      v_order.original_amount_cents, v_order.discount_amount_cents, v_order.final_amount_cents, v_order.currency);
  ELSE
    IF v_document_type = 'diagnostic_promo_cgv_acceptance_statement' OR NEW.acceptance_text IS NOT NULL THEN
      RAISE EXCEPTION 'Preuve transactionnelle Diagnostic incoherente.' USING ERRCODE = '23514';
    END IF;
  END IF;
  RETURN NEW;
...
BEGIN
  ...
  IF v_order.checkout_configuration_locked_at IS NULL THEN
    IF v_normalized_code IS NOT NULL THEN
      ...
      -- Diagnostic V1 reste payant. L exception annule aussi la reservation.
      ...
      SET promo_redemption_id = v_reservation.redemption_id,
          ...
          cgv_acceptance_statement_version_id = (SELECT id FROM public.legal_document_versions
            WHERE version = 'DIAGNOSTIC-CGV-ACCEPTANCE-PROMO-2026-09-03'
              AND document_type = 'diagnostic_promo_cgv_acceptance_statement' AND status = 'published'),
  ...
  IF v_order.promo_redemption_id IS NOT NULL THEN
    ...
    IF NOT FOUND OR v_redemption_status IS DISTINCT FROM 'reserved' OR reservation_expires_at <= now() THEN
      PERFORM private.promo_invalid();
    END IF;
  ELSE
  ...
  -- Un retry ne peut ni changer, ni ajouter, ni retirer le code deja fige.
  IF v_normalized_code IS DISTINCT FROM normalized_code THEN
    PERFORM private.promo_invalid();
  END IF;
...
BEGIN
  ...
  IF v_processing_result = 'processed' AND v_order.promo_redemption_id IS NOT NULL THEN
