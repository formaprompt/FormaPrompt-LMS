-- Excel : six offres sur le moteur commercial existant.
...
BEGIN
  IF p_checkout_intent_id IS NULL OR p_user_id IS NULL OR p_email IS NULL
    ...
    OR (p_course_id IN ('excel-initiation-inter', 'excel-perfectionnement-inter', 'excel-avance-inter')
      AND p_original_amount_cents IS DISTINCT FROM 69000)
    OR (p_course_id IN ('excel-initiation-individuel', 'excel-perfectionnement-individuel', 'excel-avance-individuel')
