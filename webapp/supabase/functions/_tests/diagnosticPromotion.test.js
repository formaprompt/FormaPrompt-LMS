...
test('retry apres erreur ambigue : expires_at Stripe reste strictement identique', () => {
  const input = { promotionApplied: true, reservationExpiresAt: '2026-08-31T10:35:00Z' };
  assert.equal(diagnosticPromotionCheckoutExpiresAt({ ...input, nowMs: Date.parse('2026-08-31T10:00:00Z') }),
    diagnosticPromotionCheckoutExpiresAt({ ...input, retry: true, nowMs: Date.parse('2026-08-31T10:02:00Z') }));
});
