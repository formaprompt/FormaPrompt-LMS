...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!data?.valid
      || data.normalized_code !== normalizedCode
      || data.original_amount_cents !== DIAGNOSTIC_IA_PAYMENT.amountTotal
      ...
      || data.discount_amount_cents + data.final_amount_cents !== DIAGNOSTIC_IA_PAYMENT.amountTotal
    ) return invalidPromotionResponse();
    ...
    return jsonResponse({
      ...
      acceptance_statement: buildDiagnosticPromoEvidence(data),
