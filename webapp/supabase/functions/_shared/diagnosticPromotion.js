...
export function diagnosticPromotionCheckoutExpiresAt({
  ...
  // Echeance stable : 35 minutes de reservation moins 4 minutes de tampon.
  // Recalculer depuis Date.now() changerait les parametres de la cle Stripe au retry.
  const stripeExpiresAt = reservationExpirySeconds - (4 * 60);
  if (
    !Number.isInteger(reservationExpirySeconds)
    || stripeExpiresAt - Math.floor(nowMs / 1000) < (retry ? 1 : 30 * 60)
